...
if not os.path.exists('./original_features'):
    os.mkdir('./original_features')
# Get image features after removing outliers
...
for i, s in enumerate(sample_sizes):
    ...
    img_data_dict[i] = img_feature[start:end]
    data_dict[i] = data_.iloc[start:end]
    start = end
# Generate data for image feature extraction and regenerate the dataset
dataset_dict = dict()
index_dict=dict()
for i in range(10):
    ith_dataset = data_dict[i]
    img_ith_dataset = img_data_dict[i]
    for j in range(9):
        dataset_key = (i+j+1) % 10
        img_ith_dataset = np.concatenate(
            (img_ith_dataset, img_data_dict[dataset_key]), axis=0)
        ith_dataset = pd.concat([ith_dataset, data_dict[dataset_key]], axis=0)
    dataset_dict[i]=ith_dataset
    index_dict[i]=list(ith_dataset.index)
    scipy.io.savemat('./original_features/{}th_original_img_features.mat'.format(i),
                     {'img_feature': img_ith_dataset, 'class': np.array(ith_dataset[targets])})
# Create a new folder to store the extracted features
if not os.path.exists('./new_features'):
    os.mkdir('./new_features')

# Train the feature extraction cnn network to run only once
# Generate feature heatmaps only run once
# file exists in features_whitegirl
# Feature maps for 10% of images randomly sampled from the training set are located in the train folder
# The test folder is the feature map of 10% of the images randomly selected in the test set
from train import Train
from generate_feature_map import Generate
for i in range(10):
    Train(i)
    Generate(i,index_dict[i])
#random forest
