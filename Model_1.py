...
print('Data reading and data preprocessing')
data = pd.read_excel('./Tabular data/merge3.xlsx')
ID = ['ID', 'SIDE']
feature_con = ['V00AGE', 'P01BMI']
feature_dis = ["V00XROSFM", 'V00XRSCFM', 'V00XRCYFM',
               'V00XRJSM', 'V00XRCHM', 'V00XROSTM',
               'V00XRSCTM', 'V00XRCYTM', 'V00XRATTM', 'V00XRKL', 'V00XROSFL',
               'V00XRSCFL', 'V00XRCYFL', 'V00XRJSL', 'V00XRCHL', 'V00XROSTL',
               'V00XRSCTL', 'V00XRCYTL', 'V00XRATTL']

targets = ['V00WOMKP']
data = data[feature_dis+targets]
...
# one-hot processing of discrete variables
from sklearn.preprocessing import MinMaxScaler
data[feature_dis] = data[feature_dis].astype(object)
data_dis = pd.get_dummies(data[feature_dis])
# Perform max-min normalization on continuous variables
data = data.drop(columns=feature_dis)
data = pd.concat([data, data_dis], axis=1)
# 
